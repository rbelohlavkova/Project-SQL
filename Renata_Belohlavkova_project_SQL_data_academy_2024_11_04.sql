...
SELECT
	...
	round(avg(wages)::NUMERIC ,0) AS avg_wages,
	round(avg(wages)/avg(price)::NUMERIC,0)*12 AS result_per_year
FROM t_renata_belohlavkova_project_sql_primary_final 
...
SELECT 
 	...
 	CASE
        ...
        ELSE round(((avg(wages) - LAG(avg(wages)) OVER (ORDER BY year)) / LAG(avg(wages)) OVER (ORDER BY year))::NUMERIC,4) * 100 
...
Dodatečná tabulka s HDP, GINI koeficientm a populace - European_Union
...
SELECT
	...
	gini
FROM economies ec
WHERE "year" BETWEEN 2006 AND 2018 AND country = 'European Union'
ORDER BY "year";
